        ...
        let
            ...
            feePadding =
                let LinearFee LinearFunction {slope = perByte} =
                    ...
                    -- Add padding to allow change and fee values to increase.
                    -- Out of caution, assume both fields can increase by the
                    -- theoretical maximum of 8 bytes each. Any overestimation
                    -- will be reduced by 'distributeSurplus'.
                    --
                    -- The only downside is that we require the user to afford
                    -- the fee for the padding, even if it later will be
                    -- optimized away.
                    --
                    -- Also cf. 'costOfIncreasingCoin'.
                    --
                    -- NOTE: This should ideally be a constant defined next to
                    -- 'costOfIncreasingCoin', but because of the package split
                    -- the overhead currently seems to big.
                    extraBytes = 16
                in
